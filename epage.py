...
class Ui_Forme(QtWidgets.QWidget):
    ...
    def setupUi(self, Form):
        ...
        self.frame.setGeometry(QtCore.QRect(20, 320, 570, 121))
        ...
        self.pline.setGeometry(QtCore.QRect(120, 40, 120, 20))
        self.pline.setObjectName("pline")
        self.kline = QtWidgets.QLineEdit(parent=self.frame)
        self.kline.setGeometry(QtCore.QRect(120, 80, 120, 20))
    ...
    def entry(self):
        ...
        key = self.kline.text()
        if key == "":
            self.showc.setText("请输入正确的十六位二进制数作为密钥！")
        else:
            self.showc.setText("密文为：")
            self.showc.append(str(aes.encrypt(plain, key, 1)))
